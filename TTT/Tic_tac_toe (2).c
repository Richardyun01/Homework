...
{
    int count = 1;
    if (dir < BOARD_SIZE) { // horizontal line
        int p = dir * BOARD_SIZE;
        for (int i = 0; i < BOARD_SIZE; i++, p++)
            count *= board[p] + 1;
    } else if (dir < BOARD_SIZE*2) { // vertical line
        int p = dir - BOARD_SIZE;
        for (int i = 0; i < BOARD_SIZE; i++, p+=BOARD_SIZE)
            count *= board[p] + 1;
    } else if (dir == BOARD_SIZE*2) { // left to right diagonal
        int p = 0;
        for (int i = 0; i < BOARD_SIZE; i++, p+=BOARD_SIZE+1)
            count *= board[p] + 1;
    } else { // right to left to diagonal
        int p = BOARD_SIZE-1;
        for (int i = 0; i < BOARD_SIZE; i++, p+=BOARD_SIZE-1)
            count *= board[p] + 1;
    }
...
int playComputer(int board[], int human_turn)
{
    ...
    // 3rd: sequence priority (working only for 3x3 board)
